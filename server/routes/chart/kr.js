...
async function fetchAlbums(date) {
  const [year, week] = refDateWeek(date, 0, 6);
...
async function fetchSingles(date) {
  ...
  const response = await nodeFetch(url, {
    method: 'post',
    body: `nationGbn=T&serviceGbn=ALL&termGbn=week&hitYear=${year}&targetTime=${week}&yearTime=3&curUrl=circlechart.kr%2Fpage_chart%2Fonoff.circle%3FserviceGbn%3DALL`,
    headers: {
      'Content-Type': 'application/x-www-form-urlencoded; charset=UTF-8',
    },
  });
  ...
  return Object.entries(List ?? {})
    .slice(0, 100)
    .map(([index, { SONG_NAME, ARTIST_NAME }]) => {
      const rank = parseInt(index, 10) + 1;
      const title = SONG_NAME.replace(/`/g, "'");
      const artist = ARTIST_NAME.replace(/`/g, "'").replace(/\|.*$/, '').trim();
      return { rank, artist, title };
    });
}
