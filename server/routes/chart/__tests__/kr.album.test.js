...
const prevData = fs.readFileSync(
  path.join(__dirname, 'html', `kr-album-${prevWeek}.html`)
);
const curData = fs.readFileSync(
  path.join(__dirname, 'html', `kr-album-${curWeek}.html`)
);
const data = { [prevWeek]: prevData, [curWeek]: curData };

const prev10 = albums.prev.kr;
const cur10 = albums.cur.kr;
...
test('fetch consecutive', async () => {
  ...
  expect(counts[0].count).toBe(133);
  ...
  expect(prevEntries[1].entry).toBe(curEntries[1].entry); // 2 -> 2
  expect(prevEntries[2].entry).toBe(curEntries[5].entry); // 3 -> 6
  expect(prevEntries[5].entry).toBe(curEntries[3].entry); // 6 -> 4
  expect(prevEntries[9].entry).toBe(curEntries[8].entry); // 10 -> 9
});
